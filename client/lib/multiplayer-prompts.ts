...
const promptSegments = [
  ...
  "Your mother was never a scribe. Steve betrayed the first circuit. Auto's fire will burn you too.",
  ...
  "When they reached the final gate, Paliko disappeared. In his place, a single black feather.",
  "Beneath it, a scroll. Inside the scroll, Ciro's last map.",
  "And so the legend passed mount to rider, whisper to song.",
  "The empire still sends Retrieval Hawks. Still paints kindness in digital ink.",
  "Still calls riders units, and Garu vectors. But now the Codex sings louder.",
  "Now the riders remember. Now they scribe in layers so deep, no algorithm can read them.",
  "And Matikah? She does not race to win. She races to keep the lies from ever crossing the finish line.",

  // Trust Messages from the Stories
  ...
  "And so the Garu riders remember. Not with monuments. Not with medals. But with rhythm, and quill, and wind.",
  "Because the empire hasn't changed. Only the riders have. And they trust no one.",
  ...
  "I'll sing it shut, she said. And then came the impossible. Ciro. Alive. Older, thinner, but unmistakable.",
  ...
  "The empire had left behind more than ruins. It had left watchers old AI fragments, data-shades of past champions.",
  "As they raced, these shadows emerged, rewriting the track with false memories.",
  "Auto's victories became imperial slogans. Matikah's songs were drowned in static. Ciro's story bent backwards, re-coded into treason.",
  "But still they rode. Every keystroke reclaimed a corner. Every note Matikah played repelled the corruption.",
  "When they reached the Final Gate the true finish line the Codex itself flickered open.",
  "Inside was the empire's master layer: a script that rewrote not just races, but history.",
  ...
  "Your score decreases mysteriously. Will you trust the rhythm-or rewrite the script?",
];

// Keep the original export for backward compatibility
export const multiplayerPrompts = promptSegments;

// Get a random multiplayer prompt - now supports combining multiple segments for longer races
export function getRandomMultiplayerPrompt(promptCount: number = 2): string {
  if (promptCount <= 1) {
    // Return a single random prompt
    return promptSegments[Math.floor(Math.random() * promptSegments.length)];
  } else {
    // Combine multiple random prompts for longer races
    const selectedPrompts: string[] = [];
    const usedIndexes = new Set<number>();
    
    // Select unique prompts to avoid repetition
    for (let i = 0; i < promptCount && usedIndexes.size < promptSegments.length; i++) {
      let randomIndex;
      do {
        randomIndex = Math.floor(Math.random() * promptSegments.length);
      } while (usedIndexes.has(randomIndex));
      
      usedIndexes.add(randomIndex);
      selectedPrompts.push(promptSegments[randomIndex]);
    }
    
    // Join prompts with a space for smooth reading flow
    return selectedPrompts.join(' ');
  }
}

// Get a prompt of appropriate length based on difficulty
export function getMultiplayerPromptByDifficulty(difficulty: 'easy' | 'medium' | 'hard' | 'extreme' = 'medium'): string {
  switch (difficulty) {
    case 'easy':
      return getRandomMultiplayerPrompt(1); // Single prompt
    case 'medium':
      return getRandomMultiplayerPrompt(2); // Two prompts combined
    case 'hard':
      return getRandomMultiplayerPrompt(3); // Three prompts combined
    case 'extreme':
      return getRandomMultiplayerPrompt(4); // Four prompts combined
    default:
      return getRandomMultiplayerPrompt(2); // Default to medium
  }
}